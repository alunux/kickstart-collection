...
class ComposeEnv:
    ...
    def setup_builder(self):
        ...
        builder_tools = ('lorax-lmc-novirt '
                         'vim-minimal '
                         'pykickstart '
                         'git')
    ...
    def compose_iso(self, iso, resultdir=None):
        ...
        if iso.fedoraver != self.fedoraver:
            print(
                'ERROR:\tFedora version is not matching in ImageIso and ComposeEnv\n'
                f'\tImageIso: Fedora {iso.fedoraver}\n'
                f'\tComposeEnv: Fedora {self.fedoraver}\n')
            sys.exit(-2)
...
def main():
    ...
    builder = ComposeEnv(fedoraver=28, march='x86_64')
